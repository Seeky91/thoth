...
Référence de mode chargée par SKILL.md (routeur) quand `/maintainability-update` est invoqué. **Pas d'audit nouveau.** Re-vérifie tous les pendings contre l'état actuel du code et met à jour les statuts. Les conventions transverses (date déterministe, écritures en delta) vivent dans SKILL.md et s'appliquent ici.
